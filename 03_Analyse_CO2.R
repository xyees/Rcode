...
# Inspect variables. Choose ONE numeric variable for the first analysis.
...
# This file is in long format: `variable` identifies each measurement and
# `values` holds its numeric observations.
variable_column <- "variable"
value_column <- "values"
# Leave as NULL to use the first available variable, or set a value printed
# below (for example, "pre", "aet", "q", or "tws").
...
# Statistics for the selected variable in the selected basin.
...
# Save the result for this one basin and one variable.
...
# Read the first one-basin file created by 02_Filter_CO2_data.R.
filtered_files <- list.files(
  out_dir,
  pattern = "^co2_one_basin_.*\\.rds$",
  full.names = TRUE
...
# 4. Map: plotted if longitude and latitude columns are available.
